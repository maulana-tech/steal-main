# CONTEXT.md — Asme Landing Page (Asset Optimization + Smooth Scroll)

> Single-page landing site. Stack: **React + TypeScript + Vite + Tailwind CSS + framer-motion + lucide-react**.
> This doc covers the asset/performance layer: lazy-loading heavy video, poster frames, Lenis smooth scroll, and scroll-driven parallax.

---

## TL;DR / Goals

1. **Faster load** — stop the browser from downloading all 5 videos at once on page load.
2. **Perceived quality** — crisp poster frames so first paint looks sharp instantly (we are NOT re-encoding or upscaling the source videos — see caveat).
3. **Lenis** — native smooth scroll, compatible with framer-motion `useInView`.
4. **Interactivity** — tasteful scroll-driven parallax on video blocks.

---

## ⚠️ Important caveat: video quality cannot be "enhanced"

The video assets live on a remote CloudFront bucket (`d8j0ntlcm91z4.cloudfront.net`, Higgsfield-generated — `hf_` prefix). They cannot be accessed, re-encoded, or upscaled from here. **Quality is already baked in at export/generation time.**

What actually makes video feel sharp + fast on the web is NOT upscaling, but:

- **Crisp poster frame** (first paint looks sharp before the video loads)
- **Correct encoding** (`+faststart`, sane bitrate)
- **Lazy-loading** (don't fetch off-screen video)

There is a real tension: *higher quality = bigger file = slower load*. The strategy below optimizes **perceived performance**, not raw bitrate.

---

## Problem #1 — every video uses `preload="auto"`

The original spec sets `preload="auto"` on **all** videos: hero + featured + philosophy + 2 service cards = **5 videos pulled simultaneously** on load, even though 4 are below the fold. This is the main cause of slow initial load.

**Fix:** only the hero is eager. Everything else lazy-loads as it enters the viewport.

---

## Implementation

### 1. `components/LazyVideo.tsx`

Reusable lazy video. `preload="none"` until an IntersectionObserver says it's near the viewport, then sets `src` and calls `load()`.
...
**Usage** — replace every below-the-fold `<video>` (Featured, Philosophy, both Service cards):

```tsx
<LazyVideo
  src="https://d8j0ntlcm91z4.cloudfront.net/.../...mp4"
...
> **Hero video is the exception.** It's above-the-fold and has its own crossfade-loop logic (canplay → fade in, timeupdate → fade out, ended → reset). Keep it eager, but still add a `poster` so there's no black flash before the video is read.

### 2. Poster frames (the part that makes it *feel* sharp + instant)

With `preload="none"`, the poster is what shows before the video loads. A good poster = crisp first paint. Pull one frame per video via ffmpeg (requires access to the source / a downloaded copy) and export to WebP:

```bash
ffmpeg -i featured.mp4 -ss 00:00:01 -vframes 1 -vf "scale=1280:-2" -q:v 80 posters/featured.webp
```

Put them in `/public/posters/`. WebP is light but sharp — far better than a blank black box while waiting for the video.

> **Open question:** do we have access to the source videos to run ffmpeg, or only the CloudFront URLs? If URLs only, posters must be captured another way (e.g. a one-off download if licensing allows, or a screenshot frame).

### 3. Encoding reference (if/when we control the source)

For any video we DO control, encode for web:

```bash
# MP4 (H.264) — universal, web-optimized
ffmpeg -i input.mov -c:v libx264 -crf 24 -preset slow -c:a aac -movflags +faststart output.mp4

# WebM (VP9) — smaller, modern browsers
ffmpeg -i input.mov -c:v libvpx-vp9 -crf 33 -b:v 0 -c:a libopus output.webm

# Background/loop video (no audio) — drop audio, scale down if oversized
ffmpeg -i input.mov -c:v libvpx-vp9 -crf 35 -b:v 0 -an -vf "scale=1920:-2" output.webm
```

- `-crf`: quality knob. Higher = smaller/worse. Safe range: VP9 `30–35`, H.264 `22–26`.
- `-movflags +faststart`: moves metadata to the front so the video can start playing before the full download. **Don't skip for web MP4.**
- Serve both formats with MP4 as fallback (`<source>` order: WebM first, MP4 second).

---

## Lenis — smooth scroll

Package is now just `lenis` (no longer `@studio-freight/lenis`).
...
> Lenis v1 smooths the **native scroll** (not a transform hack), so all existing framer-motion `useInView` triggers across the sections keep working unchanged.

---

## Interactivity — scroll parallax on video

Tasteful scroll-driven parallax that pairs with Lenis. Video drifts slowly as it scrolls through the viewport, using framer-motion `useScroll` + `useTransform`.
...
function FeaturedVideoSection() {
  ...
  return (
    <div ref={ref} className="rounded-3xl overflow-hidden aspect-video relative">
...
> `scale-110` on the inner container so the drifting video never reveals an empty edge.

---

## Apply checklist

- [ ] Replace all below-the-fold `<video>` with `<LazyVideo>` (`preload="none"` + IntersectionObserver)
- [ ] Keep hero video eager, add `poster`
- [ ] Generate a WebP poster per video into `/public/posters/`
- [ ] `npm install lenis`, add `useLenis()` at root, import `lenis/dist/lenis.css`
- [ ] (Optional) add `useScroll` parallax to hero + featured video blocks
- [ ] Resolve open question: do we have source-video access for ffmpeg poster capture?

**Highest impact:** the lazy-load + poster work (first two items) — that alone cuts initial load drastically by stopping the browser from pulling 5 videos at once.

---

## Video asset reference

| Section | URL (CloudFront) |
|---|---|
| Hero (loop + crossfade) | `.../hf_20260405_074625_a81f018a-956b-43fb-9aee-4d1508e30e6a.mp4` |
...
| Service card 1 (Strategy) | `.../hf_20260314_131748_f2ca2a28-fed7-44c8-b9a9-bd9acdd5ec31.mp4` |
| Service card 2 (Craft) | `.../hf_20260324_151826_c7218672-6e92-402c-9e45-f1e0f454bdc4.mp4` |

Base: `https://d8j0ntlcm91z4.cloudfront.net/user_38xzZboKViGWJOttwIXH07lWA1P/`
